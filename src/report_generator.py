...
def parse_command_line_args():
    ...
    if not options.config:
        ...
        print >> sys.stderr, "Please specify the config file"
        sys.exit(-1)
...
def run_hive_jobs(jobname, steps):
    ...
    if status != "COMPLETED":
        print >>sys.stderr, "Hive jobs failed"
...
def main():
    ...
    for param in params:
        ...
        print "Replace %s with %s" % (name, val)
        config_str = config_str.replace(name, val)
    config = json.loads(config_str)
        
    print "Step 1: Wait for data."
    wait_for_data(config['wait_for'], options)

    print  "Step 2: Run hive jobs and wait for completion."
    run_hive_jobs(config['name'], config['steps'])

    print "Step 3: Load data from hive to mongo using report importer."
