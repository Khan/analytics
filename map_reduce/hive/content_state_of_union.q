...
set hivevar:end_dt='2013-11-01';

INSERT OVERWRITE LOCAL DIRECTORY '/tmp/exercise_counts_2013_10_b'
SELECT
...
FROM (
    SELECT
        ...
        IF(instr(referer, "/e/") = 0 AND instr(referer, "/v/") = 0, 1, 0) AS from_dashboard,
    ...
    WHERE method = "POST"
        -- TODO(mattfaus): When/why is referer empty? How does this affect results?
        AND referer <> "-"
        AND length(trim(referer)) > 0
        -- AND substr(url, 0, length("/api/v1/user/exercises/")) = "/api/v1/user/exercises/"
...
INSERT OVERWRITE LOCAL DIRECTORY '/tmp/user_counts_2013_10_b'
SELECT
...
FROM (
    ...
    FROM (
        SELECT
            ...
            -- instr returns "position" which is 1-based indexing
            IF(instr(referer, "/e/") = 0 AND instr(referer, "/v/") = 0, 1, 0) AS math_on_dashboard,
