...
-- -d suffix=apr -d start_dt=2012-04-01 -d end_dt=2012-04-30 \
...
SELECT video_topic.topic_title,
  IF(userdata_ids.user_id RLIKE 'nouserid' OR 
     (userdata_ids.user_id IS NULL AND userdata_ids.user RLIKE 'nouserid'), 
     0, 1), 
...
FROM user_video_summary_${suffix} LEFT OUTER JOIN userdata_ids ON
  (user_video_summary_${suffix}.user = userdata_ids.user)
